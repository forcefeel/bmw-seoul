    ...
    private boolean displaySearchResult(int maxNumberOfWord){
        ...
        if(textBoxString.charAt(0) >= '0' & textBoxString.charAt(0) <= '9')
        {
            if(textBoxString.length() != 5)
            {
                ...
                textView.setText("정류장 번호 검색은 5자리 숫자 모두 입력 하셔야 합니다.");
            ...
            dr.generateNextViewFrom("http://mobile.bus.go.kr/pda/bus_information/real_station_result.jsp?station=" + keyword, keyword + " 에서 출발하는  버스 방향");
            return false;
        }
